...
fn run_analyzer(fixture: &str) -> serde_json::Value {
    ...
    let output = Command::new("cargo")
        .args(["run", "-p", "borrowscope-analyzer", "--", project_path.to_str().unwrap()])
...
fn find_var<'a>(json: &'a serde_json::Value, name: &str) -> &'a serde_json::Value {
    json["files"]["src/main.rs"].as_array().unwrap()
        .iter().find(|v| v["name"] == name)
...
fn test_split_off_detected_as_mutable() {
    ...
    let methods = v2["method_calls"].as_array().expect("v2 should have method_calls");
    let split_off = methods.iter().find(|mc| mc["method"] == "split_off")
...
fn test_retain_detected_as_mutable() {
    ...
    let methods = v["method_calls"].as_array().expect("v should have method_calls");
    let retain = methods.iter().find(|mc| mc["method"] == "retain")
...
fn test_windows_detected_as_immutable() {
    ...
    let v = find_var(&json, "v");
    let methods = v["method_calls"].as_array().expect("v should have method_calls");
    let windows = methods.iter().find(|mc| mc["method"] == "windows")
...
fn test_by_name_index_populated() {
    let json = run_analyzer("semantic_project");
    let by_name = json["by_name"].as_object().expect("by_name should be object");
...
fn test_mapped_mutex_guard_type_resolved() {
    ...
    assert!(ty.contains("MappedMutexGuard"), "Expected MappedMutexGuard, got: {}", ty);
}
...
fn test_mapped_rwlock_read_guard_type_resolved() {
    ...
    let ty = mapped_read["ty"].as_str().expect("mapped_read should have ty");
    assert!(ty.contains("MappedRwLockReadGuard"), "Expected MappedRwLockReadGuard, got: {}", ty);
}
...
fn test_mapped_rwlock_write_guard_type_resolved() {
    ...
    let ty = mapped_write["ty"].as_str().expect("mapped_write should have ty");
    assert!(ty.contains("MappedRwLockWriteGuard"), "Expected MappedRwLockWriteGuard, got: {}", ty);
}
...
fn test_mutex_lock_method_tracked() {
    ...
    let methods = mutex["method_calls"].as_array().expect("mutex should have method_calls");
    let lock = methods.iter().find(|mc| mc["method"] == "lock")
