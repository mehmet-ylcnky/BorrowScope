...
fn test_initialize_returns_capabilities() {
    ...
    assert!(result["capabilities"]["textDocumentSync"]["openClose"]
        .as_bool()
        .unwrap());
...
fn test_multiple_files_tracked_independently() {
    ...
    // a unchanged
    let resp_a = server.request(
        "borrowscope/debug/fileContent",
        serde_json::json!({"uri": "file:///tmp/a.rs"}),
    );
    assert_eq!(resp_a["result"]["content"], "fn a() {}");
    // b changed
    let resp_b = server.request(
        "borrowscope/debug/fileContent",
        serde_json::json!({"uri": "file:///tmp/b.rs"}),
    );
...
fn test_empty_file_content() {
    ...
    let resp = server.request(
        "borrowscope/debug/fileContent",
        serde_json::json!({"uri": "file:///tmp/empty.rs"}),
    );
    assert_eq!(resp["result"]["content"], "");
}
...
fn test_large_file_content() {
    ...
    let resp = server.request(
        "borrowscope/debug/fileContent",
        serde_json::json!({"uri": "file:///tmp/large.rs"}),
    );
    assert_eq!(
        resp["result"]["content"].as_str().unwrap().len(),
        content.len()
    );
}
...
fn test_unicode_content() {
    ...
    let resp = server.request(
        "borrowscope/debug/fileContent",
        serde_json::json!({"uri": "file:///tmp/unicode.rs"}),
    );
    assert_eq!(resp["result"]["content"], content);
}
...
fn test_rapid_changes_all_applied() {
    ...
    let resp = server.request(
        "borrowscope/debug/fileContent",
        serde_json::json!({"uri": "file:///tmp/rapid.rs"}),
    );
...
fn test_reopen_file_updates_content() {
    ...
    let resp = server.request(
        "borrowscope/debug/fileContent",
        serde_json::json!({"uri": "file:///tmp/reopen.rs"}),
    );
...
fn test_notification_after_request() {
    ...
    let resp = server.request(
        "borrowscope/debug/fileContent",
        serde_json::json!({"uri": "file:///tmp/mix.rs"}),
    );
...
fn test_ownership_graph_error_has_message() {
    ...
    assert!(
        resp["result"]["_status"].is_string(),
        "Should have _status field"
    );
}
...
fn test_analysis_updated_sent_after_change() {
    ...
    // Now change the file
    let notifs = server.notify_and_collect(
        "textDocument/didChange",
        serde_json::json!({
    ...
    let analysis_notifs: Vec<_> = notifs
        .iter()
        .filter(|n| n["method"] == "borrowscope/analysisUpdated")
        .collect();
    assert!(
        !analysis_notifs.is_empty(),
        "Should send analysisUpdated after didChange. Got: {:?}",
        notifs
    );
}
...
fn test_analysis_updated_has_timestamp() {
    ...
    let notif = notifs
        ...
        .unwrap();
    assert!(
        notif["params"]["timestamp"].is_number(),
...
fn test_analysis_updated_not_sent_for_non_rust() {
    ...
    assert!(
        analysis_notifs.is_empty(),
        "Should NOT send analysisUpdated for non-Rust files"
    );
}
...
fn test_analysis_updated_not_sent_on_did_open() {
    ...
    assert!(
        analysis_notifs.is_empty(),
        "didOpen should NOT trigger analysisUpdated"
    );
}
...
fn test_analysis_updated_multiple_changes_each_sends() {
    ...
    // First change
    let notifs1 = server.notify_and_collect(
        "textDocument/didChange",
        serde_json::json!({
    ...
    // Second change
    let notifs2 = server.notify_and_collect(
        "textDocument/didChange",
        serde_json::json!({
    ...
    assert!(notifs1
        .iter()
        .any(|n| n["method"] == "borrowscope/analysisUpdated"));
    assert!(notifs2
        .iter()
        .any(|n| n["method"] == "borrowscope/analysisUpdated"));
}
...
fn test_analysis_updated_functions_list_updates() {
    ...
    assert!(
        names.contains(&"two"),
        "Should include newly added function 'two'. Got: {:?}",
        names
    );
}
...
fn test_analysis_updated_only_affected_functions() {
    ...
    assert!(
        fn_names.contains(&"changed"),
        "Should list 'changed'. Got: {:?}",
        fn_names
    );
    assert!(
        !fn_names.contains(&"unchanged"),
        "'unchanged' should NOT be listed. Got: {:?}",
        fn_names
    );
}
...
fn test_analysis_updated_no_notification_if_no_ownership_change() {
    ...
    assert!(
        analysis_notifs.is_empty(),
        "Should NOT send notification when content is unchanged. Got: {:?}",
        analysis_notifs
    );
}
...
fn test_analysis_updated_comment_only_change_no_notification() {
    ...
    assert!(
        analysis_notifs.is_empty(),
        "Comment-only change should NOT trigger notification. Got: {:?}",
        analysis_notifs
    );
}
...
fn test_diagnostics_not_sent_without_workspace() {
    ...
    for n in &diag_notifs {
        ...
        assert!(
            diags.is_empty(),
            "Without workspace, diagnostics should be empty"
        );
    }
}
...
fn test_diagnostics_not_sent_for_non_rust() {
    ...
    assert!(
        ...
        "Non-rust files should not get diagnostics"
    );
}
...
fn test_code_lens_returns_array() {
    ...
    let resp = server.request(
        "textDocument/codeLens",
        serde_json::json!({"textDocument": {"uri": "file:///tmp/cl.rs"}}),
    );
...
fn test_code_lens_empty_without_workspace() {
    ...
    let resp = server.request(
        "textDocument/codeLens",
        serde_json::json!({"textDocument": {"uri": "file:///tmp/cl2.rs"}}),
    );
    let lenses = resp["result"].as_array().unwrap();
    assert!(
        lenses.is_empty(),
        "Without workspace, codeLens should be empty (semantic only)"
    );
}
...
fn test_code_lens_no_lenses_for_empty_file() {
    ...
    let resp = server.request(
        "textDocument/codeLens",
        serde_json::json!({"textDocument": {"uri": "file:///tmp/cl_empty.rs"}}),
    );
    let lenses = resp["result"].as_array().unwrap();
...
fn test_code_lens_before_workspace_ready_returns_empty() {
    ...
    let resp = server.request(
        "textDocument/codeLens",
        serde_json::json!({"textDocument": {"uri": "file:///tmp/cl3.rs"}}),
    );
    assert!(resp["result"].is_array());
    assert!(resp["result"].as_array().unwrap().is_empty());
}
...
fn test_inlay_hint_empty_without_workspace() {
    ...
    assert!(
        hints.is_empty(),
        "Without workspace, inlayHint should be empty (semantic only)"
    );
}
...
fn test_incremental_analysis_updated_sent_on_change() {
    ...
    assert!(
        !analysis_notifs.is_empty(),
        "Should send analysisUpdated after change"
    );
}
...
fn test_incremental_unchanged_function_not_in_notification() {
    ...
    if let Some(notif) = analysis_notif {
        ...
        assert!(
            !has_unchanged,
            "Unchanged function should not be in notification. Got: {:?}",
            functions
        );
    }
}
...
fn test_debounce_rapid_changes_single_notification() {
    ...
    assert!(
        analysis_notifs.len() <= 1,
        "Rapid changes should produce at most 1 notification, got {}",
        analysis_notifs.len()
    );
}
...
fn test_debounce_notification_after_pause() {
    ...
    let early_analysis: Vec<_> = early_notifs
        .iter()
        .filter(|n| n["method"] == "borrowscope/analysisUpdated")
        .collect();
    assert!(
        early_analysis.is_empty(),
        "Should NOT send notification before debounce expires"
    );
    ...
    let late_analysis: Vec<_> = late_notifs
        .iter()
        .filter(|n| n["method"] == "borrowscope/analysisUpdated")
        .collect();
    assert!(
        !late_analysis.is_empty(),
        "Should send notification after debounce expires"
    );
}
...
fn test_debounce_configurable_via_init_options() {
    ...
    assert!(
        !analysis.is_empty(),
        "Custom debounce (50ms) should have fired by 150ms"
    );
}
...
fn test_cache_server_responsive_after_change() {
    ...
    assert!(
        ...
        "Server should respond even during debounce"
    );
}
...
fn test_perf_debounce_plus_notification_within_budget() {
    ...
    assert!(
        ...
        "Full pipeline should be < 600ms, got {:?}",
        elapsed
    );
}
...
fn test_perf_rapid_requests_dont_block() {
    ...
    assert!(
        ...
        "5 rapid requests should be < 1s, got {:?}",
        elapsed
    );
}
...
fn test_memory_cache_created_on_open() {
    ...
    let resp = server.request(
        "borrowscope/debug/fileContent",
        serde_json::json!({"uri": "file:///tmp/mem1.rs"}),
    );
    assert_eq!(resp["result"]["content"], "fn test() { let x = 1; }");
}
...
fn test_memory_open_file_not_evicted() {
    ...
    let resp = server.request(
        "borrowscope/debug/fileContent",
        serde_json::json!({"uri": "file:///tmp/mem_open.rs"}),
    );
