...
Static variables (`static` and `static mut`) and const items (`const`) present unique challenges. These are module-level items, not local variables within function bodies. The `#[trace_borrow]` attribute is designed for function instrumentation and does not have visibility into module-level declarations.

Additionally, static initialization in Rust has complex semantics—statics may be initialized lazily, may involve const evaluation, and `static mut` access is always unsafe. Properly tracking these would require a different macro approach, potentially a module-level attribute or a custom static declaration macro.

### Async Function Limitations

While async functions can be annotated with `#[trace_borrow]` and basic tracking works, the macro does not currently track async-specific operations like future creation, polling, or await points. Async functions in Rust are transformed by the compiler into state machines, and the relationship between the original code and the generated state machine is complex.

The runtime library provides async tracking functions (`track_future_create`, `track_future_poll`, etc.), but integrating these into the macro would require understanding the async desugaring process, which happens after macro expansion. This remains an area for future development.
