...
fn benchmark_basic_tracking() {
    ...
    let overhead_per_op =
        (tracking_duration.as_nanos() - baseline_duration.as_nanos()) / (iterations * 6); // 6 operations per iteration
...
fn benchmark_complex_patterns() {
    ...
    for i in 0..1000 {
        ...
        let _rc2 = track_rc_clone(
            &format!("rc2_{}", i),
            &format!("rc1_{}", i),
            std::rc::Rc::clone(&rc1),
        );
        let _rc3 = track_rc_clone(
            &format!("rc3_{}", i),
            &format!("rc1_{}", i),
            std::rc::Rc::clone(&rc1),
        );
        ...
        {
            let _borrow1 = track_refcell_borrow(
                &format!("borrow1_{}", i),
                &format!("cell_{}", i),
                "benchmark",
                cell.borrow(),
        ...
        {
            let _borrow_mut = track_refcell_borrow_mut(
                &format!("borrow_mut_{}", i),
                &format!("cell_{}", i),
                "benchmark",
                cell.borrow_mut(),
            );
...
fn benchmark_disabled_tracking() {
    ...
    println!(
        "No-tracking baseline (1M iterations): {:?}",
        no_tracking_time
    );
    println!(
        "Per operation: {} ns",
        no_tracking_time.as_nanos() / iterations
    );
