...
fn main() {
    ...
    for rate in rates {
        ...
        println!(
            "   Sample rate {:.0}%: {} events recorded ({:.1}%)",
            rate * 100.0,
            events.len(),
            percentage
        );
    ...
    let events = get_events();
    println!(
        "   Total events: {} (3 critical + ~10 sampled)",
        events.len()
    );
    println!(
        "   Config: {}, User: {}, Result: {}",
        config, user_id, result
    );
    ...
    let events = get_events();
    println!(
        "   100 borrows at 5% rate: {} events recorded",
        events.len()
    );
    ...
    println!(
        "   Speedup: {:.1}x",
        full_time.as_nanos() as f64 / sampled_time.as_nanos() as f64
    );
