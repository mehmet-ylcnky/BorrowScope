...
import { MergedVariable, RuntimeInfo, AwaitCrossing } from "./merge-views";
import { StaticVariable } from "./runtime-mapper";

/** All possible divergence kinds */
export type DivergenceKind =
  | "conditional_move"      // Static predicted move, runtime didn't move (branch not taken)
  | "unexpected_move"       // Static didn't predict move, runtime moved
  | "rc_leak"              // Rc/Arc never dropped, ref_count > 0
  | "rc_cycle"             // Rc ref count went up then never back to 0
  | "borrow_count_mismatch" // Static predicted N borrows, runtime had M
  | "type_mismatch"        // Runtime type differs from static type
  | "unexpected_drop"      // Dropped earlier than static scope suggests
  | "missing_drop"         // Never dropped (non-Copy, non-moved)
  | "unsafe_hidden"        // Unsafe code hides ownership info from static
  | "async_borrow_held"    // Borrow held across await point
  | "refcell_panic"        // RefCell borrow violation at runtime
  | "double_move"          // Variable moved twice (shouldn't happen in safe code)
  | "use_after_move"       // Events after move (shouldn't happen in safe code)
  | "weak_upgrade_fail"    // Weak::upgrade returned None (dangling weak ref)
  | "lock_contention"      // Lock held for unusually long time
...
export function detectAllDivergences(
  ...
  const divergences: DetailedDivergence[] = [];
  const s = merged.static_info;
  const r = merged.runtime_info;
  if (!r) return divergences;
  ...
  const weakUpgradeFails = findEventsForVar(merged.var_id, allEvents, "WeakUpgrade")
    .filter(e => eventData(e).success === false);
  ...
  if (r.was_actually_moved) {
    ...
    if (moveTimestamp > 0) {
      const eventsAfterMove = (merged as any).events?.filter(
        (e: RuntimeEvent) => eventData(e).timestamp > moveTimestamp && eventType(e) !== "Drop"
      ) || [];
...
export function analyzeDivergences(merged: MergedVariable[], allEvents: RuntimeEvent[]): {
  total: number;
...
function findEventsForVar(varId: string | null, events: RuntimeEvent[], type: string): RuntimeEvent[] {
  if (!varId) return [];
  return events.filter(e => {
    if (eventType(e) !== type) return false;
...
function findMoveTimestamp(varId: string | null, events: RuntimeEvent[]): number {
  if (!varId) return 0;
  for (const e of events) {
    if (eventType(e) === "Move" && eventData(e).from_id === varId) {
      return eventData(e).timestamp;
    }
  }
  return 0;
}
