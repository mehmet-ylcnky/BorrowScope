...
describe("12.10 Async Borrow Tracking", () => {
  ...
  it("reports borrow_across_await issue", () => {
    ...
    const result = tracker.trackAsyncBorrows(events);
    assert.ok(result.issues.some((i: any) => i.kind === "borrow_across_await"));
    assert.ok(result.issues.find((i: any) => i.kind === "borrow_across_await").severity === "warning");
  });
  ...
  it("reports mut_borrow_across_await as error", () => {
    ...
    const result = tracker.trackAsyncBorrows(events);
    assert.ok(result.issues.some((i: any) => i.kind === "mut_borrow_across_await"));
    assert.ok(result.issues.find((i: any) => i.kind === "mut_borrow_across_await").severity === "error");
  });
  ...
  it("reports multiple_awaits_crossed", () => {
    ...
    const result = tracker.trackAsyncBorrows(events);
    assert.ok(result.issues.some((i: any) => i.kind === "multiple_awaits_crossed"));
  });
  ...
  it("reports long_held_borrow (>100ms)", () => {
    ...
    const result = tracker.trackAsyncBorrows(events);
    assert.ok(result.issues.some((i: any) => i.kind === "long_held_borrow"));
  });
