...
describe("5.2 Graph Data Model", () => {
  ...
  function sampleData(overrides: any = {}): any {
    return {
  ...
  it("mutable borrow creates mutable_borrow edge", () => {
    ...
    const edge = result.edges.find((e: any) => e.target === "m");
    assert.ok(edge);
  ...
  it("move creates move edge", () => {
    ...
    const result = model.buildGraphModel(data);
    const edge = result.edges.find((e: any) => e.kind === "move");
  ...
  it("Rc clone creates rc_clone edge", () => {
    ...
    const result = model.buildGraphModel(data);
    const edge = result.edges.find((e: any) => e.kind === "rc_clone");
    assert.ok(edge);
  ...
  it("Arc clone creates arc_clone edge", () => {
    ...
    const result = model.buildGraphModel(data);
    const edge = result.edges.find((e: any) => e.kind === "arc_clone");
    assert.ok(edge);
  ...
  it("auto-creates nodes referenced in edges but not in variables", () => {
    ...
    const result = model.buildGraphModel(data);
    const unknownNode = result.nodes.find((n: any) => n.id === "unknown_ref");
  ...
  it("deduplicates identical edges", () => {
    ...
    const result = model.buildGraphModel(data);
    const borrowEdges = result.edges.filter((e: any) => e.kind === "shared_borrow");
  ...
  it("multiple borrows from same source create multiple edges", () => {
    ...
    const result = model.buildGraphModel(data);
    const borrowEdges = result.edges.filter((e: any) => e.source === "data");
  ...
  it("complex graph with borrows, moves, and clones", () => {
    ...
    assert.strictEqual(result.edges.filter((e: any) => e.kind === "shared_borrow").length, 1);
    assert.strictEqual(result.edges.filter((e: any) => e.kind === "move").length, 1);
    assert.strictEqual(result.edges.filter((e: any) => e.kind === "rc_clone").length, 1);
  });
  ...
  it("node size scales with edge count", () => {
    ...
    const result = model.buildGraphModel(data);
    const dataNode = result.nodes.find((n: any) => n.id === "data");
    const lonelyNode = result.nodes.find((n: any) => n.id === "lonely");
