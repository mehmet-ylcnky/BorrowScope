...
export function parseLocation(location: string | undefined | null): SourceLocation | null {
...
export function findStaticMatch(
...
): { match: StaticVariable | null; confidence: MappedVariable["match_confidence"] } {
  // 1. Exact: name + line + type all match
  if (runtimeLine !== null && runtimeType) {
    const exact = staticVars.find(
      (v) => v.name === runtimeName && v.line === runtimeLine && typesMatch(v.type_display, runtimeType)
    );
  ...
  if (runtimeLine !== null) {
    ...
    if (byLine) return { match: byLine, confidence: "name_line" };
  }

  // 3. Name + type match (for shadowed variables where line differs)
  if (runtimeType) {
    const byType = staticVars.find(
      (v) => v.name === runtimeName && typesMatch(v.type_display, runtimeType)
    );
    if (byType) return { match: byType, confidence: "name_type" };
  }

  // 4. Name only (last resort)
  const byName = staticVars.find((v) => v.name === runtimeName);
  if (byName) return { match: byName, confidence: "name_only" };

  return { match: null, confidence: "none" };
}

/** Check if two type strings are compatible (handles short vs full paths) */
function typesMatch(staticType: string, runtimeType: string): boolean {
  if (staticType === runtimeType) return true;
  // "Vec<i32>" matches "alloc::vec::Vec<i32>"
  if (runtimeType.endsWith(staticType)) return true;
  if (staticType.endsWith(runtimeType)) return true;
...
function extractVarInfo(event: RuntimeEvent): {
  var_id: string | null;
...
export function mapVariables(
  ...
  const varMap = new Map<string, { var_name: string; type_name: string | null; location: string | null; events: RuntimeEvent[] }>();
  ...
  for (const [varId, info] of varMap) {
    ...
    const { match, confidence } = findStaticMatch(
      info.var_name,
      loc?.line ?? null,
      info.type_name,
      staticVars
...
export function mappingStats(mapped: MappedVariable[]): {
  total: number;
  exact: number;
  name_line: number;
  name_type: number;
  ...
  for (const m of mapped) {
    if (m.match_confidence === "exact") stats.exact++;
    else if (m.match_confidence === "name_line") stats.name_line++;
    else if (m.match_confidence === "name_type") stats.name_type++;
    else if (m.match_confidence === "name_only") stats.name_only++;
