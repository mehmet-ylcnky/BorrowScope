...
function showWelcomePanel(context) {
    const panel = vscode.window.createWebviewPanel("borrowscopeWelcome", "Welcome to BorrowScope", vscode.ViewColumn.One, { enableScripts: true });
...
async function checkPrerequisites() {
    ...
    const serverPath = vscode.workspace.getConfiguration("borrowscope").get("server.path", "");
...
function getWelcomeHtml(status, context) {
    const check = (ok) => ok ? "✅" : "❌";
    return `<!DOCTYPE html>
<html><head><style>
