...
describe("12.6 Divergence Detection", () => {
    ...
    it("detects Rc leak (never dropped, ref_count > 0)", () => {
        ...
        assert.ok(divs.some((d) => d.kind === "rc_leak"));
        assert.ok(divs.find((d) => d.kind === "rc_leak").severity === "error");
    });
    ...
    it("detects Rc cycle (multiple clones, never freed)", () => {
        ...
        const divs = detector.detectAllDivergences(m, []);
        assert.ok(divs.some((d) => d.kind === "rc_cycle"));
    });
    // === missing_drop ===
    it("detects missing drop for non-Copy owned var", () => {
        ...
        const divs = detector.detectAllDivergences(m, []);
        assert.ok(divs.some((d) => d.kind === "missing_drop"));
        assert.ok(divs.find((d) => d.kind === "missing_drop").severity === "warning");
    });
    ...
    it("detects borrow held across await", () => {
        ...
        const divs = detector.detectAllDivergences(m, []);
        assert.ok(divs.some((d) => d.kind === "async_borrow_held"));
        const d = divs.find((d) => d.kind === "async_borrow_held");
    ...
    it("detects unsafe hidden for non-RawPtr var", () => {
        ...
        const divs = detector.detectAllDivergences(m, []);
        assert.ok(divs.some((d) => d.kind === "unsafe_hidden"));
        assert.ok(divs.find((d) => d.kind === "unsafe_hidden").severity === "info");
    });
    ...
    it("detects conditional move (owned, not moved, no borrows)", () => {
        ...
        const divs = detector.detectAllDivergences(m, []);
        assert.ok(divs.some((d) => d.kind === "conditional_move"));
        assert.ok(divs.find((d) => d.kind === "conditional_move").severity === "info");
    });
    ...
    it("detects Weak::upgrade failure", () => {
        ...
        assert.ok(divs.some((d) => d.kind === "weak_upgrade_fail"));
        assert.ok(divs.find((d) => d.kind === "weak_upgrade_fail").severity === "warning");
    });
    ...
    it("detects channel receive failure", () => {
        ...
        const divs = detector.detectAllDivergences(m, events);
        assert.ok(divs.some((d) => d.kind === "channel_recv_fail"));
    });
    // === use_after_move ===
    it("detects use after move", () => {
        ...
        assert.ok(divs.some((d) => d.kind === "use_after_move"));
        assert.ok(divs.find((d) => d.kind === "use_after_move").severity === "error");
    });
...
//# sourceMappingURL=divergence-detector.test.js.map