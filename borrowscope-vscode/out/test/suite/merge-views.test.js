...
describe("12.4 Merge Static + Runtime Views", () => {
    ...
    it("mergeViews includes static_only for unmatched static vars", () => {
        const events = [{ New: { timestamp: 10, var_name: "data", var_id: "data_1", type_name: "Vec<i32>" } }];
        const merged = merge.mergeViews(staticVars, events);
        const staticOnly = merged.filter((m) => m.agreement === "static_only");
    ...
    it("mergeViews includes runtime_only for unmatched runtime vars", () => {
        ...
        const merged = merge.mergeViews(staticVars, events);
        const runtimeOnly = merged.find((m) => m.agreement === "runtime_only");
    ...
    it("mergeViews computes match for agreeing data", () => {
        ...
        const merged = merge.mergeViews(staticVars, events);
        const dataVar = merged.find((m) => m.name === "data" && m.var_id === "data_1");
    ...
    it("computes actual_lifetime_ns from New to Drop", () => {
        ...
        const merged = merge.mergeViews(staticVars, events);
        const dataVar = merged.find((m) => m.var_id === "data_1");
    ...
    it("tracks ref_count_peak for Rc", () => {
        ...
        const merged = merge.mergeViews(staticVars, events);
        const rcVar = merged.find((m) => m.var_id === "rc_1");
    ...
    it("counts unsafe accesses", () => {
        ...
        const ptrVar = merged.find((m) => m.var_id === "ptr_1");
    ...
    it("detects Rc leak (never dropped, ref_count > 0)", () => {
        ...
        assert.ok(rcVar.divergences.some((d) => d.kind === "rc_leak"));
    });
    it("detects missing drop for non-Copy owned var", () => {
        ...
        const merged = merge.mergeViews(statics, events);
        const sVar = merged.find((m) => m.var_id === "s_1");
        assert.strictEqual(sVar.agreement, "diverge");
        assert.ok(sVar.divergences.some((d) => d.kind === "missing_drop"));
    });
    it("detects async borrow held across await", () => {
        ...
        assert.ok(dataVar.divergences.some((d) => d.kind === "async_borrow_held"));
    ...
    it("detects unsafe hidden info", () => {
        ...
        const merged = merge.mergeViews(statics, events);
        const vVar = merged.find((m) => m.var_id === "v_1");
        assert.ok(vVar.divergences.some((d) => d.kind === "unsafe_hidden"));
    });
    it("no divergence for normal owned var with drop", () => {
        ...
        const merged = merge.mergeViews(statics, events);
        const xVar = merged.find((m) => m.var_id === "x_1");
    ...
    it("no divergence for Copy type without drop", () => {
        ...
        const merged = merge.mergeViews(statics, events);
        const nVar = merged.find((m) => m.var_id === "n_1");
        // Copy types don't need explicit drop
        assert.strictEqual(nVar.divergences.filter((d) => d.kind === "missing_drop").length, 0);
    });
    // === mergeSummary ===
    ...
    it("handles empty events", () => {
        ...
        assert.ok(merged.every((m) => m.agreement === "static_only"));
    });
    ...
    it("handles Arc events same as Rc", () => {
        ...
        const merged = merge.mergeViews(statics, events);
        const aVar = merged.find((m) => m.var_id === "a_1");
    ...
    it("handles WeakNew with weak_count tracking", () => {
        ...
        const merged = merge.mergeViews(statics, events);
        const wVar = merged.find((m) => m.var_id === "w_1");
    ...
    it("handles Box events", () => {
        ...
        const merged = merge.mergeViews(statics, events);
        const bVar = merged.find((m) => m.var_id === "b_1");
        assert.strictEqual(bVar.agreement, "match");
    });
});
//# sourceMappingURL=merge-views.test.js.map