...
describe("12.11 Configuration and Toggle UI", () => {
    ...
    it("package.json has toggleRuntime command", () => {
        ...
        assert.ok(cmds.some((c) => c.command === "borrowscope.toggleRuntime"));
    });
    it("package.json has runtime.enable command", () => {
        const cmds = pkgJson.contributes.commands;
        assert.ok(cmds.some((c) => c.command === "borrowscope.runtime.enable"));
    });
    it("package.json has runtime.disable command", () => {
        const cmds = pkgJson.contributes.commands;
        assert.ok(cmds.some((c) => c.command === "borrowscope.runtime.disable"));
    });
    it("package.json has runtime.toggleTimings command", () => {
        const cmds = pkgJson.contributes.commands;
        assert.ok(cmds.some((c) => c.command === "borrowscope.runtime.toggleTimings"));
    });
    it("package.json has runtime.toggleDropOrder command", () => {
        const cmds = pkgJson.contributes.commands;
        assert.ok(cmds.some((c) => c.command === "borrowscope.runtime.toggleDropOrder"));
    });
    it("package.json has runtime.toggleRefCounts command", () => {
        const cmds = pkgJson.contributes.commands;
        assert.ok(cmds.some((c) => c.command === "borrowscope.runtime.toggleRefCounts"));
    });
    it("package.json has runtime.toggleDivergences command", () => {
        const cmds = pkgJson.contributes.commands;
        assert.ok(cmds.some((c) => c.command === "borrowscope.runtime.toggleDivergences"));
    });
    it("package.json has runtime.setSource command", () => {
        const cmds = pkgJson.contributes.commands;
        assert.ok(cmds.some((c) => c.command === "borrowscope.runtime.setSource"));
    });
...
//# sourceMappingURL=runtime-status.test.js.map