...
describe("4.2 Language Client Configuration", () => {
    ...
    it("throws error when binary not found", () => {
        ...
        try {
            assert.throws(() => resolveServerPath({ extensionPath: "/nonexistent", configuredPath: "" }), /borrowscope-lsp binary not found/);
        }
        finally {
    ...
    it("error message mentions borrowscope.server.path", () => {
        ...
        try {
            let msg = "";
            try {
                resolveServerPath({ extensionPath: "/x", configuredPath: "" });
            }
            catch (e) {
                msg = e.message;
            }
