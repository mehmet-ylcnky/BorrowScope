...
describe("12.9 WebSocket Live Connection", () => {
    ...
    before(() => {
        ...
        global.WebSocket = class MockWebSocket {
            constructor(url) {
                this.url = url;
                this.readyState = 0;
                setTimeout(() => { this.readyState = 1; if (this.onopen)
                    this.onopen(); }, 0);
            }
            close() { this.readyState = 3; if (this.onclose)
                this.onclose(); }
            send() { }
        };
    ...
    it("handles single event message", (done) => {
        ...
        socket.onStateChanged((state) => {
            if (state === "connected") {
                ...
                socket.ws.onmessage({ data: JSON.stringify(event) });
            }
        });
        socket.connect(9876);
    });
    it("handles batch event message", (done) => {
        ...
        socket.onStateChanged((state) => {
            if (state === "connected") {
                ...
                socket.onBatchReceived((events) => {
                    assert.strictEqual(events.length, 2);
                    assert.strictEqual(socket.eventCount, 2);
                    socket.dispose();
                    done();
                });
                socket.ws.onmessage({ data: JSON.stringify(batch) });
            }
        });
        socket.connect(9876);
    });
    it("ignores invalid JSON messages", (done) => {
        ...
        socket.onStateChanged((state) => {
            if (state === "connected") {
                socket.ws.onmessage({ data: "{broken json" });
                assert.strictEqual(socket.eventCount, 0);
                socket.dispose();
                done();
            }
    ...
    it("ignores invalid event structure", (done) => {
        ...
        socket.onStateChanged((state) => {
            if (state === "connected") {
                socket.ws.onmessage({ data: JSON.stringify({ FakeEvent: { timestamp: 1 } }) });
                assert.strictEqual(socket.eventCount, 0);
                socket.dispose();
                done();
            }
    ...
    it("accepts all 88 event types via WebSocket", (done) => {
        ...
        socket.onStateChanged((state) => {
            if (state === "connected") {
                ...
                socket.ws.onmessage({ data: JSON.stringify(events) });
...
//# sourceMappingURL=runtime-socket.test.js.map