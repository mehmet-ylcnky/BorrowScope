...
exports.buildRefCountData = buildRefCountData;
function buildRefCountData(graph) {
    ...
    for (const v of variables) {
        if (v.ownership_category === "Rc" || v.ownership_category === "Arc") {
            ...
            const isClone = clones.some((c) => c.clone_variable === v.name);
            if (!isClone) {
                families.set(v.name, {
                    name: v.name,
    ...
    for (const [source, series] of families) {
        const cloneVars = clones.filter((c) => c.source_variable === source);
