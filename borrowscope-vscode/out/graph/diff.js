...
exports.computeGraphDiff = computeGraphDiff;
function computeGraphDiff(oldModel, newModel) {
    if (!oldModel) {
        return {
    ...
    for (const newNode of newModel.nodes) {
        ...
        if (oldNode.category !== newNode.category)
            changes.category = newNode.category;
        if (oldNode.type !== newNode.type)
            changes.type = newNode.type;
        if (oldNode.isAlive !== newNode.isAlive)
            changes.isAlive = newNode.isAlive;
        if (oldNode.size !== newNode.size)
            changes.size = newNode.size;
