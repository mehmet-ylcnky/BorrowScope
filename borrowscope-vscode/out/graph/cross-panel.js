...
exports.CrossRefPanel = void 0;
const vscode = __importStar(require("vscode"));
class CrossRefPanel {
    static createOrShow(extensionUri, data, d3Uri) {
        ...
        const panel = vscode.window.createWebviewPanel("borrowscopeCrossRef", "BorrowScope: Cross-References", column, { enableScripts: true, retainContextWhenHidden: true, localResourceRoots: [vscode.Uri.joinPath(extensionUri, "media")] });
    ...
    _buildHtml(data, d3Uri) {
        ...
        for (const b of crossBorrows) {
            for (const seg of b.path) {
                ...
                if (!fileSet.has(file))
                    fileSet.set(file, { count: 0, isCurrentFile: false });
                if (file !== currentFile)
...
exports.CrossRefPanel = CrossRefPanel;
function esc(s) {
    return s.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
}
//# sourceMappingURL=cross-panel.js.map